...
- cv-craft 0.4.0: rules re-grounded in a 2026 evidence pass (new EVIDENCE.md lists every source by tier). The rubric's keyword category becomes requirement evidence (term named and shown in a bullet), replacing an unsourced 80% verbatim threshold. Export renders DOCX and PDF and picks per submission. Length allows two pages at 5 to 7 years only when content fills them and never a partial page. check asks about knockout questions and a referral path before any CV work. tailor records a defense for every metric and offers a cover letter. screen-prep leads with logistics knockouts and adds metric defense, a two-branch salary rule, and an authenticity check. New linkedin-align mode. verify-pdf.py flags text addressed to AI screeners and a partly filled last page, and no longer crashes on a cp1252 console.
