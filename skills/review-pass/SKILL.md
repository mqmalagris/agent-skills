...
3. **Confirm** — user says go / edit / abort. Accept partial edits ("skip verify", "add security-audit").
...
6. **Act** — if there are actionable findings, ask how to apply them (skip this if the verdict is clean):
...
| **implementation-review** | 3 | Always, for a real change. Seven parallel checks; **its check 7 already runs security-audit**, so this covers the baseline security pass. |
...
- Mid-build task with no diff yet — use /dev-flow (it runs these same stages in the full chain).
