...
1. **Detect tier** from the task description (table below). If ambiguous, ask one question: which tier.
2. **Print the plan** — the ordered stage list for that tier, each stage marked RUN or SKIP with a one-line reason. Honor each sub-skill's own skip rules (e.g. grill-me skips single-file changes; to-prd skips bug fixes). Reflect those in the SKIP reasons.
3. **Confirm** — user says go / edit / abort. Accept partial edits ("skip the PRD", "add adhd"). Record exactly what they approved.
4. **Drive** — invoke each RUN stage via the Skill tool, in order. After each returns, checkpoint: report what it produced (file path, decision, diff), then start the next. Stop and surface if a stage can't proceed (fuzzy scope → route back to grill-me; unsettled architecture → route to compass).
5. **Close** — after the last stage, one-line summary of artifacts produced (PRD path, ADR path, plan path, PR URL).
...
| **feature** | multi-file, some design surface, user-visible | grill-me → to-prd → heist → code → verify → code-review → pr-craft |
| **architecture** | new subsystem, public API, hard-to-reverse decision, schema, "how should I structure X" | adhd → grill-me → to-prd → compass → heist → code → verify → code-review → pr-craft |
...
- **maestro** — only if there are multiple independent plans worth parallelizing. Not in the default chains; add on request.
- **verify** — only if the change has runtime surface to drive. Skip docs/test-only diffs.
- **code-review** — run for anything beyond a trivial one-liner.
